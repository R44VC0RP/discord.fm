...
import { parseElevenLabsSpeechSpeed } from './tts.js';
...
export function loadConfig(): AutomationConfig {
  ...
  const generationEnabled = bool('AUTOMATION_GENERATION_ENABLED', false);
  ...
  const config: AutomationConfig = {
    ...
    crossfadeMs: int('AUTOMATION_CROSSFADE_MS', 6000, 500, 10_000),
    badwords,
    ...
    elevenLabsSpeechSpeed: parseElevenLabsSpeechSpeed(process.env.ELEVENLABS_SPEECH_SPEED, 'ELEVENLABS_SPEECH_SPEED', generationEnabled),
    ...
    // Zero explicitly disables the corresponding UTC-day circuit breaker.
    // Keep invalid/negative values fail-closed rather than silently enabling it.
    djDailyToolLimit: int('AUTOMATION_DJ_DAILY_TOOL_LIMIT', 0, 0, 100_000),
    djDailyModelTokenLimit: int('AUTOMATION_DJ_DAILY_MODEL_TOKEN_LIMIT', 0, 0, 100_000_000),
    ttsDailyCharacterLimit: int('AUTOMATION_TTS_DAILY_CHARACTER_LIMIT', 0, 0, 10_000_000),
